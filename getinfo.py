import requests, re, json, MySQLdb, time
  ...
  for album  in data['topalbums']['album']:
    ...
    if name != "(null)":
      ...
      for userinfo  in data:
        ...
        d=db.cursor()
        d.execute("""SELECT id  FROM lastfm_album WHERE name =%s""", [name])
        res=d.fetchone()
        alid=res[0]
        b=db.cursor()
  ...
  for album  in data:
    ...
    for userinfo  in data:
      ...
      if t==1:
        ...
        c=db.cursor()
        c.execute( """Update """+str(uid)+"""_album SET playcount=%s WHERE id=%s""", [int(count), alid])
        db.commit()
          ...
          #print("deleted "+str(alid)+" from "+user)
